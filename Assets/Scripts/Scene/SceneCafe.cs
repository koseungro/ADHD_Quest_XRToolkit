...
"You have to press the button on the trigger button when the same shape as before appears the second time."); });
    ...
    protected override void Start()
    {
        ...
        IsAssetLoad(delegate { EnterFunc(); });
        //EnterFunc();
        //UIManager.Inst.FullFadeOut();
        //videoPlayer.LoadVideoInfo();
        //videoPlayer.SetVideoURL(videoPlayer.videoInfoArray[(int)VideoType.CAFEALONE1].GetVideoPath);
        ...
        UIManager.Inst.FullFadeIn(
            delegate {
                UIManager.Inst.CompleteAndCallback(func, delegate { UIManager.Inst.FullFadeOut(); },
                                                         ...
                                                         delegate { UIManager.Inst.ShowNarrationText("It's a weekday morning, so the café is empty.\n"+
        ...
        UIManager.Inst.FullFadeIn(
            delegate {
                UIManager.Inst.CompleteAndCallback(func, delegate { UIManager.Inst.FullFadeOut(); },
                                                         ...
                                                         delegate { UIManager.Inst.ShowNarrationText("It's a weekday afternoon and there are quite a few people.\n" +
        ...
        UIManager.Inst.FullFadeIn(
            delegate
            {
                UIManager.Inst.CompleteAndCallback(func, delegate { UIManager.Inst.FullFadeOut(); },
                                                         ...
                                                         //delegate { UIManager.Inst.ShowNarrationText("처음보여지는 도형을 잘 기억하시고, 다른 도형이 나타날 때 \n테블릿 화면 위에 조이스틱 버튼을 누르세요", 0, -80f); },
                                                         delegate { UIManager.Inst.ShowNarrationText("I came to a cafe to study with my friend on a weekend afternoon.\n"+
